...
import { CELL_SYSTEM_PROMPT } from './cell';
import { MOLECULAR_SYSTEM_PROMPT } from './molecular';
...
2. 逐步骤判断正误并给出反馈
3. 总结评价
4. 提取涉及的知识点（使用简短、稳定的知识点名称）
5. 判断题目难度（1-10）
6. 如果作答有问题，归纳一个主要错误类型，例如审题不清、概念混淆、公式错误、计算失误、方法错误、表达不完整
7. 评估理解、推理、计算、应用能力（强/中/弱）${BLOCK_INSTRUCTION}`,
  gradeEssay: `请批改以下作文：
1. 给出总分和各维度得分
2. 列出优点
3. 列出不足
...
2. 列出具体任务，每项包含学科、知识点、预估时间、优先级、原因`,
  ...
  cell: CELL_SYSTEM_PROMPT,
  molecular: MOLECULAR_SYSTEM_PROMPT,
};
